  ...
  module Jobs
    class InterViewProfessionalsController < BaseController
      ...
      include ProcessInstanceHelper
    	...
    	def initialize
        @widget_request = true
        @custom_process_embedded = CustomProcess.where(name: 'interviewer_professionals').order(:created_at).last
      end
      ...
      def show
        ...
        if @widget_request
          render :partial => "guara/jobs/inter_view_professionals/widget_edit", :locals => { vacancy: @vacancy, :custom_process_embedded=> @custom_process_embedded}
        else
          render
        end
    	end
